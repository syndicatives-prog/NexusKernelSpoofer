...
static NTSTATUS Hooked(HANDLE KeyHandle, PUNICODE_STRING ValueName,
    ...
    if (KeyValueInformationClass == KeyValuePartialInformation || KeyValueInformationClass == KeyValueFullInformation) {
        ...
        if (partial->Type == REG_SZ && partial->DataLength >= 2 && ValueName && ValueName->Buffer) {
            ...
            if (_wcsicmp(val, L"SystemProductName") == 0) RtlStringCbCopyW((PWSTR)partial->Data, partial->DataLength, L"FakeProduct");
            else if (_wcsicmp(val, L"SystemManufacturer") == 0) RtlStringCbCopyW((PWSTR)partial->Data, partial->DataLength, L"FakeManufacturer");
            else if (_wcsicmp(val, L"SystemSerialNumber") == 0) RtlStringCbCopyW((PWSTR)partial->Data, partial->DataLength, L"FakeSerial");
            else if (_wcsicmp(val, L"MachineGuid") == 0) RtlStringCbCopyW((PWSTR)partial->Data, partial->DataLength, L"{Fake-Machine-GUID}");
            else if (_wcsicmp(val, L"HardwareProfileGuid") == 0) RtlStringCbCopyW((PWSTR)partial->Data, partial->DataLength, L"{Fake-HW-Profile}");
            else if (_wcsicmp(val, L"ProductId") == 0) RtlStringCbCopyW((PWSTR)partial->Data, partial->DataLength, L"FakeProductId");
        }
    }
    return status;
}
